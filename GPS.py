                ...
                if (chkVal == int(chkSum, 16)): # Compare the calculated checksum with the one in the NMEA sentence
                     ...
                     if (gpsChars.startswith('$GNGGA') or gpsChars.startswith('$GNGLL') or gpsChars.startswith('$GNVTG')):
                         if (gpsChars.startswith('$GNGGA')):
                             flag = 0
                             str1 = gpsChars.split(',')[4]
                             if (str1 == ""):
                                 GPIO.output(ledPin, GPIO.HIGH)
                                 time.sleep(0.5)
                                 #print("Finding GPS signal")
...
time_prev = a
file1 = open("/home/pi/Desktop/Senior_Design_Project/GPS_data.txt", "a")
file1.write('Start Time: %s\n' %(a))
while True:
    current_time = datetime.datetime.now()
    running_time = current_time - time_prev
    time_prev = current_time
    file2 = open("/home/pi/Desktop/Senior_Design_Project/GPS_data.txt", "a")
    file2.write('Running Time = %s' %(running_time))
    count += 1
    if count == 1:
        print("GPS start")
    readGPS()
